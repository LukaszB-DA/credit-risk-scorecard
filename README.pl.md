...
- **Pipeline pozyskania:** skrypt [`fetch_data.py`](scripts/fetch_data.py) pobiera dane, przekodowuje
  target i zapisuje surowy plik → CSV zaimportowany do Excela. Surowe dane są dołączone do repo
  jako [`credit_data.csv`](data/credit_data.csv) (1000 × 21, bez braków danych).
...
| `Documentation` | Dwujęzyczne (EN/PL) notatki: metodyka binningu i selekcji zmiennych |
...
Skoroszyt modelu: [`Credit_risk_model.xlsx`](model/Credit_risk_model.xlsx).
