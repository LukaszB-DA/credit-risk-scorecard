...
- **Ingestion pipeline:** the [`fetch_data.py`](scripts/fetch_data.py) script downloads the data,
  recodes the target and writes the raw file → the CSV is imported into Excel. The raw pull is
  included in this repo as [`credit_data.csv`](data/credit_data.csv) (1,000 × 21, no missing values).
...
| `Documentation` | Bilingual (EN/PL) binning & variable-selection methodology notes |
...
The model workbook: [`Credit_risk_model.xlsx`](model/Credit_risk_model.xlsx).
